...
def is_valid_polis_address(address, network='mainnet'):
    ...
    polis_version = 140 if network == 'testnet' else 55
    ...
    try:
        ...
        # rescue from exception, not a valid Polis address
        return False

    if (address_version != polis_version):
        return False
...
# shims 'til we can fix the polisd side
def SHIM_serialise_for_polisd(sentinel_hex):
    ...
    # superblock => "trigger" in polisd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # polisd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    polisd_hex = serialise(obj)
    return polisd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('polis.conf')
