...
class PolisConfig():
    ...
    def slurp_config_file(self, filename):
        # read polis.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is polis.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 24127 if (network == 'mainnet') else 124127

        # use default port for network if not specified in polis.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from polis.conf
