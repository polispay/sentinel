...
def valid_polis_address(network='mainnet'):
    return 'TEoLJfxDuAkWSFLyryURZaYeoconQXStfH' if (network == 'testnet') else 'PGQuyQ6rtG5XSVe1bXbR7PdghpAZQEYdus'


@pytest.fixture
def invalid_polis_address(network='mainnet'):
    return 'TEoLJfxDuAkWSFLyryURZaYeoconQXStfr' if (network == 'testnet') else 'PGQuyQ6rtG5XSVe1bXbR7PdghpAZQEYdut'
...
def mn_list():
    ...
    masternodelist_full = {
        u'701854b26809343704ab31d1c45abc08f9f83c5c2bd503a9d5716ef3c0cda857-1': u'  ENABLED 70201 TJhVdM8t7VeyUE74bfwUpCVwrhMGRBhpow 1474157572    82842 1474152618  71111 52.90.74.124:24130',
        u'f68a2e5d64f4a9be7ff8d0fbd9059dcd3ce98ad7a19a9260d1d6709127ffac56-1': u'  ENABLED 70201 TPmoAx7HAuqRFuCYvBkQn8qzvoBCYQ91zQ 1474157732  1590425 1474155175  71122 [2604:a880:800:a1::9b:0]:24130',
        u'656695ed867e193490261bea74783f0a39329ff634a10a9fb6f131807eeca744-1': u'  ENABLED 70201 TQZENgwGCzcQ2ZQe6HUfgM3YpriUoMYhFf 1474157704   824622 1474152571  71110 178.62.203.249:24130',
    }
...
def test_valid_polis_address():
    from polislib import is_valid_polis_address

    main = valid_polis_address()
    test = valid_polis_address('testnet')

    assert is_valid_polis_address(main) is True
    assert is_valid_polis_address(main, 'mainnet') is True
    assert is_valid_polis_address(main, 'testnet') is False

    assert is_valid_polis_address(test) is False
    assert is_valid_polis_address(test, 'mainnet') is False
    assert is_valid_polis_address(test, 'testnet') is True


def test_invalid_polis_address():
    from polislib import is_valid_polis_address

    main = invalid_polis_address()
    test = invalid_polis_address('testnet')

    assert is_valid_polis_address(main) is False
    assert is_valid_polis_address(main, 'mainnet') is False
    assert is_valid_polis_address(main, 'testnet') is False

    assert is_valid_polis_address(test) is False
    assert is_valid_polis_address(test, 'mainnet') is False
    assert is_valid_polis_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from polislib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % polislib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(polislib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(polislib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(polislib.blocks_to_seconds(16616)) == 2612035
